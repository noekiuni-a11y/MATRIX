  ...
  return (
    <div className="matrix-bg min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white border-4 border-slate-900 rounded-3xl shadow-[8px_8px_0px_#0F172A] p-8 animate-pop-in">
        ...
        <p className="text-slate-600 font-medium mt-1">Log in to grab your Brix.</p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          {error && (
            <p data-testid="login-error" className="text-sm font-bold text-red-600 bg-red-50 border-2 border-red-300 rounded-lg px-3 py-2">
              {error}
            </p>
          )}
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white font-black px-6 py-3 border-4 border-slate-900 rounded-xl shadow-[4px_4px_0px_#0F172A] hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_#0F172A] transition-all disabled:opacity-60"
          >
        ...
        <p className="text-center text-sm font-medium text-slate-600 mt-6">
          New here?{" "}
          <Link to="/register" className="font-black text-blue-600 underline" data-testid="goto-register">
            Create an account
          </Link>
        </p>
      </div>
